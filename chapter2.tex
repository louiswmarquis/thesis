...
We want $m$ to be large enough and $\epsilon_z$ small enough that this quantity is at most $\epsilon$. We can calculate the thresholds $m^*$ and $\epsilon_z^*$ above which this is the case. To make the calculations simpler, we can restrict both terms to at most $\frac{\epsilon}{2}$.
...
\begin{equation}
    \begin{split}
        m^*_{H} \approx 2\log_2(n) + 2\log_2\left(\frac{n\log_2{n}\times 0.01}{10^{-6}}\right) + 4 \approx 4\log_2(n) + 2\log_2(\log_2(n)) + 18
    \end{split}
\end{equation}

\begin{equation}
    \begin{split}
        \tilde{N}_{T} &= 21m(n)n - 70n + 98m(n)^2 - 18m(n) + 4m(n)\log_2(\frac{2m(n)}{\epsilon})
    \end{split}
\end{equation}

We plot this $T$-count of the ``coherent method'' as well as that of the ``expansion method'' in \eqref{eq: expansionTs} as functions of $n$. It turns out that the expansion method surpasses the coherent method in $T$-count at approxmiately $n \approx 122$. At $n = 122$, the coherent method requires about $3.94 * 10^5$ $T$-gates and has $m = 52$, while the expansion method requires about $4.00 * 10^5$ $T$-gates.
...
The asymptotic advantage of the coherent method ($O(n^3\log_2{n})$) over the expansion method ($O(n^4)$) is only useful in $U_A^{(r)}$ if the number of $T$ gates in $U_R(R^{(r)})$ can also be reduced from $O(n^2\log_2(\frac{n}{\epsilon}))$. Otherwise, the latter complexity dominates and maintains the overall complexity unchanged.

In Chapter 3, we explore chemical systems and Hamiltonians for which it is the case that $U_R(R^{(r)})$ can be decomposed ``easily'' as such.
