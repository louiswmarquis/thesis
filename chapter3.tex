...
Observe that the coherent method has the lowest asymptotic upper bound among all three methods for low-rank $K$-range Hamiltonians, as long as $K = o(n)$ (otherwise it's the same as the expansion method). Of course, this is assuming that $K$ is not so low that $H_{2e}$ simplifies into a sum of commuting terms (which comprise one Trotter term) in the way of \eqref{eq: collapse}.
For generic-rank $K$-range $H_{2e}$, it performs asymptotically better than brute force if $K = \omega(n^{0.25})$ and better than expansion if $K = o(n)$. 
...
\section{Extraction of Low-Range Terms from Generic Electronic Structure Hamiltonians}

We have shown some asymptotic advantages for the coherent method within a deterministic Trotterization assumption. qDRIFT is often stated to perform better for Hamiltonians with many small terms (such as our case of $H_{2e}$), so we would like to see how effective the coherent method may be in the context of such randomized algorithms as well. Asymptotic order-of-magnitude calculations are not as straightforward in the context of qDRIFT, however, as its gate count is based on the weights of the terms, which varies across specific Hamiltonians and systems. Instead, we propose an algorithm for decomposing generic Hamiltonians into low-range terms and numerically assess its performance.
